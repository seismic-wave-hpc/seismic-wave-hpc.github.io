...
::: {.callout-tip}
Miyabiでは他の東大情報基盤センターのシステムとは異なり，ホームディレクトリからもジョブの実行ができます．一方で，後述するようにホームディレクトリのファイル数制限が厳しいため，やはりジョブ実行はワークディレクトリから行うことを勧めます．
:::
...
`module`は多くの大型計算機に用いられる環境切り替えのためのコマンドです．`module load`によりコンパイラやライブラリなどを切り替えることができます．
Miyabi-Gを使う上ではほとんどの場合NVIDIA環境で